...
def snap(x_mm: float, step_mm: float) -> float:
    """Snap x to nearest step (e.g. 44.9 -> 45.0 for step 5)."""
    step = float(step_mm)
    if step <= 0:
        return float(x_mm)
    return float(step * round(float(x_mm) / step))
...
def nearest_metric(value_mm: float, gauges_mm) -> tuple[float, float]:
    """
    Return (nearest_gauge_mm, error_mm) where error_mm = measured - nominal.
    Example: measured 5.55 vs gauge 6.0 -> err = -0.45
    """
    gauges = [float(g) for g in gauges_mm] if gauges_mm is not None else []
    if not gauges:
        raise ValueError("No gauge candidates provided (use --gauges)")
    best = min(gauges, key=lambda g: abs(float(value_mm) - g))
    return float(best), float(float(value_mm) - best)
...
def nearest_metric(value_mm: float, gauges_mm: list[float] | tuple[float, ...]):
    """
    Return (nearest_gauge_mm, error_mm) where error_mm = measured - nominal.
    Example: measured 5.55 vs gauge 6.0 -> err = -0.45
    """
    if gauges_mm is None:
        raise ValueError("gauges_mm is None")
    gauges = [float(g) for g in gauges_mm]
    if len(gauges) == 0:
        raise ValueError("No gauge candidates provided")
    best = min(gauges, key=lambda g: abs(value_mm - g))
    return best, float(value_mm - best)
...
def measure_screw(mask: np.ndarray,
    ...
    # Estimate screw thickness in pixels so we can clamp erosion and avoid "mask vanished" frames
    cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not cnts:
        return None
    c0 = max(cnts, key=cv2.contourArea)
    (_, _), (w0, h0), _ = cv2.minAreaRect(c0)
    short_px = max(1.0, float(min(w0, h0)))

    base_len_px = int(max(0, round(erode_len_mm * pxmm_local)))
    base_dia_px = int(max(0, round(erode_dia_mm * pxmm_local)))

    # Clamp erosions relative to thickness (critical for reliability)
    max_len_px = int(short_px * 0.12)   # keep length mask usable
    max_dia_px = int(short_px * 0.18)   # keep diameter mask usable
    base_len_px = min(base_len_px, max_len_px)
    base_dia_px = min(base_dia_px, max_dia_px)

    # Retry schedule: if a frame is thin/noisy, back off erosion instead of failing
    attempts = [
        (base_len_px, base_dia_px),
        (max(0, base_len_px - 1), max(0, base_dia_px - 2)),
        (max(0, base_len_px - 2), max(0, base_dia_px - 3)),
        (0, max(0, base_dia_px - 4)),
        (0, 0),
    ]

    k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))

    for er_len_px, er_dia_px in attempts:
        m_len = mask.copy()
        m_dia = mask.copy()

        if er_len_px > 0:
            m_len = cv2.erode(m_len, k, iterations=er_len_px)
        if er_dia_px > 0:
            m_dia = cv2.erode(m_dia, k, iterations=er_dia_px)

        # Enough pixels?
        n_len = int(np.count_nonzero(m_len))
        n_dia = int(np.count_nonzero(m_dia))
        if n_len < 250 or n_dia < 250:
            continue
        ...
        # Dynamic binning: bin width in mm (THIS is the stability fix)
        bw = max(0.35, float(bin_mm))
        nbins = int(np.clip(total_len / bw, 60, 200))
        min_pts = 10
        need_bins = max(18, int(nbins * 0.22))

        # Diameter profile (from diameter mask)
        ...
        centers_d, widths_d = binned_widths(t_d, s_d, tmin, tmax, nbins=nbins, min_pts=min_pts)
        valid_d = ~np.isnan(widths_d)
        if int(np.count_nonzero(valid_d)) < need_bins:
            continue
        ...
        # Length profile (from length mask) for underhead boundary
        centers_l, widths_l = binned_widths(t_len, s_len, tmin, tmax, nbins=nbins, min_pts=min_pts)
        valid_l = ~np.isnan(widths_l)
        if int(np.count_nonzero(valid_l)) < need_bins:
            continue

        tn_l = (centers_l[valid_l] - tmin) / (total_len + 1e-9)
        w_l = median_filter_1d(widths_l[valid_l], 5)

        left_mean = float(np.mean(w_l[tn_l < 0.12])) if np.any(tn_l < 0.12) else float(np.mean(w_l[:6]))
        right_mean = float(np.mean(w_l[tn_l > 0.88])) if np.any(tn_l > 0.88) else float(np.mean(w_l[-6:]))
        ...
        streak_need = max(4, int(nbins * 0.03))
        boundary = None
        run = 0
    ...
    if debug:
        print("DEBUG: measurement retries exhausted (thin/unstable mask this frame).")
    return None
...
def main():
    ...
    ap.add_argument("--bin-mm", type=float, default=0.65, help="Width-profile bin size in mm (larger = more stable)")
